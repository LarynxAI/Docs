...
Jeżeli klasyfikacja nie jest możliwa, ze względu na niejednoznaczność obrazu albo złą jakość zdjęcia należy wybrać opcję `Nieklasyfikowalne`. Wiąże się to z koniecznością uzasadnienia swojej decyzji w oknie tekstowym, które pojawi się po wybraniu tej opcji. Po napisaniu uzasadnienia można wcisnąć przycisk `Oznacz jako nieklasyfikowalne` aby przejść do dalszego oznaczania zdjęć.
...
5. *Grupy* - sekcja pozwalająca na zarządzanie grupami użytkowników (obecnie nie używana)
6. *Użytkownicy* - sekcja pozwalająca na zarządzanie użytkownikami, ich hasłami oraz uprawnieniami

== Przesyłanie zbioru obrazów do oznaczenia
Aby utworzyć i przesłać zbiór danych (obrazów) do oznaczania, należy przejść do sekcji *Datasets* i kliknąć przycisk *Dodaj dataset*.
#figure(
...
Oraz przesłać (pojedynczo przyciskiem _Dodaj kolejne zdjęcie_ lub zbiorowo poprzez sekcję _Przesyłanie zbiorcze_) obrazy do oznaczenia.
...
Na koniec należy kliknąć przycisk *Zapisz*. Zbiór danych zostanie dodany do listy zbiorów danych.
